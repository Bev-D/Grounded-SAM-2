...
def process_picture_API(
    ...
    output_dir: str = "./outputs/picturetest",
    ...
    if save_visualizations:
        ...
        cv2.imwrite(os.path.join(output_dir, img_stem+"groundingdino_annotated_image.jpg"), annotated_frame)

        mask_annotator = sv.MaskAnnotator()
        annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)

        cv2.imwrite(os.path.join(output_dir, img_stem+"grounded_sam2_annotated_image_with_mask.jpg"), annotated_frame)

    # 保存 JSON
    if dump_json_results:
        with open(os.path.join(output_dir, img_stem+"grounded_sam2_local_image_demo_results.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="./outputs/picturetest",
    ...
    args = parser.parse_args([
        "--img-path", "http://10.60.9.26:8000/A0160.JPG",
        "--text-prompt", "boat."
    ])
